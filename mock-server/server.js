...
function handleVerifyCustomer(args) {
  ...
  const success = VALID_CODES.has(String(verification_code).trim());
  if (success) {
    return {
      status: "success",
      verified: true,
...
app.get("/health", (req, res) => {
  res.json({ status: "ok", service: "kapture-mock-server", timestamp: new Date().toISOString() });
});
...
app.post("/webhook", (req, res) => {
  ...
  try {
    const results = toolCalls.map((toolCall) => {
      ...
      console.log("[webhook] Incoming tool call:", JSON.stringify(maskedRequest));

      const result = executeTool(toolName, args);
      const maskedResponse = maskPayload({ toolCallId, result });
      console.log("[webhook] Tool result:", JSON.stringify(maskedResponse));
    ...
    console.error("[webhook] Tool execution error:", error.message);
